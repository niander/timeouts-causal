...
PrintTableOnes <- function(df, xvars = .(slope.before, seconds, quarter, margin), print.it = F) {
  tabones <- list()
  i <- 1
...
PrintTableOnesUnify <- function(df, xvars = .(slope.before, seconds, quarter, margin), print.it = F) {
  df %<>%
  ...
  tabones %<>% set_names(config$deltas)
  if (print.it) print(str_glue("==========================="))
  return(invisible(tabones))
}
...
BrewTableOne <- function(tab.ones, suffix.name) {
  ...
  seconds (mean (sd)) & <%= tab[3,1] %> & <%= tab[3,2] %> & <%= tab[3,3] %> \\\\
  quarter (mean (sd)) & <%= tab[4,1] %> & <%= tab[4,2] %> & <%= tab[4,3] %> \\\\
...
BrewBigTableOne <- function(tab.ones, suffix.name) {
  ...
  brew(file=path(rprojroot::find_root(rprojroot::is_rstudio_project), "reports", "tab.one.subtable.brew"),
     output =  path("reports", str_c("latex.table", suffix.name, "txt", sep = ".")))
}
...
all.tab.ones <- all.mat %>%
  bind_rows(mat.nobal %>% mutate(method = as_factor("unmatch"))) %>%
  get_all_tab_ones()
...
get_controls <- . %>%
  ...
  select(-time, -pts, -valid.infl.score)

controls.unmatched <- games.inflscore %>% 
  add_seconds() %>%
  get_controls() %>%
  mutate(A = 0, treatment = "control")

create_unmatched_by_poss <- function(data, myposs) {
  data.t <- filter(data, A == 1, poss == myposs)
  
  #data.c <- filter(data, A == 0) %>%
  #  filter(strat %in% pull(data.t, strat))
  
  data.c <- filter(data, A == 0)
  
  bind_rows(data.t, data.c) %>%
    mutate(match.id = NA, match.poss = myposs)
}

mat.unmatch <- mat.nobal %>%
  filter(A == 1) %>%
  bind_rows(controls.unmatched) %>%
  mutate(method = "unmatch")

mat.unmatch.home <- mat.unmatch %>% create_unmatched_by_poss("home")
mat.unmatch.away <- mat.unmatch %>% create_unmatched_by_poss("away")

# home
all.tab.ones.home <- all.mat %>%
  filter(match.poss == "home") %>%
  bind_rows(mat.unmatch.home) %>%
  get_all_tab_ones()

BrewBigTableOne(all.tab.ones.home, str_c("all.methods", year, "home", sep = "."))

# away
all.tab.ones.home <- all.mat %>%
  filter(match.poss == "away") %>%
  bind_rows(mat.unmatch.away) %>%
  get_all_tab_ones()

BrewBigTableOne(all.tab.ones.home, str_c("all.methods", year, "away", sep = "."))
...
last5min.permuts <- last5min.data %>%
  dlply(.(method, delta, match.poss), RunInferPermutation, .progress = "text") %>%
        #.parallel = T) %>%
  RIPListToDataFrame()

SaveObject(last5min.permuts, folder = path("reports", "data"), name = "last5min.permutations.10m")  

last5min.results.table <- last5min.permuts %>%
  GetPValues(get.ci = F, filter.home = F, separate.effects = T)

last5min.results.table

BrewTableResults(last5min.results.table, "last5min.results.10m", themethods = last5min.methods)
```


## All but last 5 minutes
```{r}
but5min.methods <- c(str_c(c("nobal", "maha",  "propensity.nocalip"), ".but5min"))
JoinAllMatchesAndDeltaWithData(years = c("2017"),
                               methods = but5min.methods,
                               data = res.timeout.effect.data)
but5min.data <- env_get_list(global_env(), nms = str_c("mat", but5min.methods, sep = ".")) %>%
  modify(~ drop_na(., match.id)) %>%
  set_names(but5min.methods) %>%
  bind_rows(.id = "method") %>%
  mutate(method = fct_relevel(method, !!!but5min.methods))

but5min.data %<>%
  filter(season == 2017) %>%
  mutate(infl.score = if_else(match.poss == "away", -infl.score, infl.score))

but5min.permuts <- ldply(config$deltas, .fun = function(d) {
    inform(str_c("Start delta = ", d))
    perm.delta <- but5min.data %>%
      filter(delta == d) %>%
      dlply(.(method, delta, match.poss), RunInferPermutation, .progress = "text")  %>% 
      RIPListToDataFrame()
    inform(str_c("Done delta = ", d))
    return(perm.delta)
  }) %>%
  as_tibble()

# but5min.permuts <- but5min.data %>%
#   dlply(.(method, delta, match.poss), RunInferPermutation, .progress = "text") %>%
#         #.parallel = T) %>%
#   RIPListToDataFrame()

SaveObject(but5min.permuts, folder = path("reports", "data"), name = "but5min.permutations.10m")  

but5min.results.table <- but5min.permuts %>%
  GetPValues(get.ci = F, filter.home = F, separate.effects = T)

but5min.results.table

BrewTableResults(but5min.results.table, "but5min.results.10m", themethods = but5min.methods)
```
